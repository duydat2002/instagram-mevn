...
const imgStyle = computed(() => {
  ...
  } else if (currentTab.value.tab == "FilterPost") {
    cursorType = "pointer";
  } else {
...
<template>
  <div class="flex flex-nowrap w-full h-full">
    <div class="relative w-[400px] h-full select-none">
      ...
      <div v-if="currentTab.tab == 'CropPost'" class="absolute left-0 bottom-0 w-full select-none">
        <div class="left">
          <div
    ...
    <transition name="slideRight">
      <div
        v-if="['FilterPost', 'CaptionPost'].includes(currentTab.tab)"
        class="border-l border-borderColor overflow-hidden"
      >
        <FilterPost v-if="currentTab.tab == 'FilterPost'" @drawCanvas="drawCanvas" />
        <CaptionPost v-if="currentTab.tab == 'CaptionPost'" />
      </div>
    </transition>
  </div>
</template>
